...
const imagesDirectory = './static/PortraitH2/Top'; // Chemin vers le dossier des images

// Supprimer le contenu existant du fichier imagesList.js
fs.writeFileSync('./public/imagesListAccH2.js', '', 'utf-8');
...
fs.readdir(imagesDirectory, (err, files) => {
  ...
  const data = imageFiles.map(imageFile => (`/PortraitH2/Top/${imageFile}`));

  // Convertir la liste en JSON
  const imagesList = JSON.stringify(data);

  // Écrire la liste d'images dans le fichier imagesList.js
  fs.appendFileSync('./public/imagesListAccH2.js', `export const images = ${imagesList};`, 'utf-8');
});
