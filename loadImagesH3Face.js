...
const imagesDirectory = './static/accPortraitH3/Face'; // Chemin vers le dossier des images
console.log("")

// Supprimer le contenu existant du fichier imagesList.js
fs.writeFileSync('./public/imagesListH3Face.js', '', 'utf-8');
...
fs.readdir(imagesDirectory, (err, files) => {
  ...
  const data = imageFiles.map(imageFile => (`/accPortraitH3/Face/${imageFile}`));

  // Convertir la liste en JSON
  const imagesList = JSON.stringify(data);

  // Écrire la liste d'images dans le fichier imagesList.js
  fs.appendFileSync('./public/imagesListH3Face.js', `export const images = ${imagesList};`, 'utf-8');
});
