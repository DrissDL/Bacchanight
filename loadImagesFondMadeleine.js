...
const imagesDirectory = './static/Madeleine/fond/'; // Chemin vers le dossier des images
console.log("")

// Supprimer le contenu existant du fichier imagesList.js
fs.writeFileSync('./public/images/fond/imagesListFondMadeleine.js', '', 'utf-8');
...
fs.readdir(imagesDirectory, (err, files) => {
  ...
  const data = imageFiles.map(imageFile => (`/Madeleine/fond/${imageFile}`));

  // Convertir la liste en JSON
  const imagesList = JSON.stringify(data);

  // Écrire la liste d'images dans le fichier imagesList.js
  fs.appendFileSync('./public/images/fond/imagesListFondMadeleine.js', `export const images = ${imagesList};`, 'utf-8');
});
