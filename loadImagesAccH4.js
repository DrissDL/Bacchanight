...
const imagesDirectory = './static/PortraitH4/Top'; // Chemin vers le dossier des images

// Supprimer le contenu existant du fichier imagesList.js
fs.writeFileSync('./public/imagesListAccH4.js', '', 'utf-8');
...
fs.readdir(imagesDirectory, (err, files) => {
  ...
  const data = imageFiles.map(imageFile => (`/PortraitH4/Top/${imageFile}`));

  // Convertir la liste en JSON
  const imagesList = JSON.stringify(data);

  // Écrire la liste d'images dans le fichier imagesList.js
  fs.appendFileSync('./public/imagesListAccH4.js', `export const images = ${imagesList};`, 'utf-8');
});
